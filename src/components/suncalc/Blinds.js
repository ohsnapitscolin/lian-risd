...
import { useProgress, usePhases } from "../../hooks/suncalc";
...
export default function Blinds({ slide, hide }) {
  const momentProgress = useProgress();
  const { current, next } = usePhases();

  if (!momentProgress || !current || !next) return null;
